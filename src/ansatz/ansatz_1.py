...
class Layer1Ansatz(IAnsatz):
    """
    layer_1 do notebook:
      - Para cada qubit i: RX(W[d,i,0]); RZ(W[d,i,1])
      - Entrelaçamento anel (CRX) com parâmetros W[d,*,2]:
          CRX(W[d,0,2]) em (n-1 -> 0)
          Para j = n-1 .. 1: CRX(W[d,j,2]) em (j -> j-1)
    """
    ...
    def apply(self, weights, wires: Sequence[int]) -> None:
        ...
        for d in range(D):
            ...
            if n >= 2:
                # CRX anel
                qml.CRX(W[d, 0, 2], wires=(wires[-1], wires[0]))
                for j in range(n - 1, 0, -1):
                    qml.CRX(W[d, j, 2], wires=(wires[j], wires[j - 1]))
